                            ...
                            <div class="table-responsive">
                                <table class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer"
                                    ...
                                    <tr class="odd" v-for="(data,i) in data_content.data" :key="data.id">
                                        ...
                                        <td>
                                            Projects : {{data.projects_count}} <br>
                                            Admin : {{data.users_count}} <br>
                                        </td>
...
export default {
    ...
    methods: {
        loadDataContent(page = 1) {
            ...
            this.loader = true
            this.authGet('institutions', this.filter)
                .then((data) => {
        ...
        addData() {
            this.disabled = true;
            this.form.post(this.base_api + 'institutions', this.setHeader())
                .then(({data}) => {
                    this.disabled = false;
                    if (data.status) {
        ...
        updateData() {
            this.disabled = true;
            this.form.patch(this.base_api + 'institutions/' + this.form.id, this.setHeader())
                .then(({data}) => {
                    this.disabled = false;
                    if (data.status) {
                        $('#kt_modal_add_customer').modal('hide')
        ...
        deleteData() {
            if (confirm('Delete data?')) {
                this.disabled = true;
                this.form.delete(this.base_api + 'institutions/' + this.form.id, this.setHeader())
                    .then(({data}) => {
                        this.disabled = false;
                        if (data.status) {
                            $('#kt_modal_add_customer').modal('hide')
